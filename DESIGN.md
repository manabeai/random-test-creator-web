...
入力行は54pxのガター、内容幅に沿う数式、54pxの末端操作で構成し、82pxの行高で実際の改行と横方向の空白を保つ。空の入力では最初の挿入点だけを示し、一行以上ある時のbelow挿入点は最終行の左ではなく記法開始位置に揃えてその直下へ置く。rightは記号列の末端、inside／variantは入れ子や分岐の局所操作としてRustの投影方向と画面上の位置を一致させる。選択後のインスペクタはその記号を含む行グループの直下へ置き、三角形のアンカーで対象との関係を失わせない。

820px以下では58pxの固定ツールバーと58pxの表示切り替えを積み、入力形式／生成ケースのどちらか一方だけを表示する。切り替えは一回の操作で完了し、入力画面の状態を保つ。ガターは38px、入力行は74px、ツールバー操作は42px。名前候補は作成ポップオーバーと同じ面の下段に置き、ホバーに依存せず、一文字入力欄のフォーカス中だけ5列のグリッドとして展開する。各候補は44px以上にする。インスペクタは左右2pxまで広がり、基本型・横方向・縦方向は縦積みへ変わる。430px以下ではガター、挿入操作、seed欄をさらに詰めるが、入力形式の改行は変えない。
...
- **Insertion Point:** 追加位置の「＋」は44px四方、ディープインクの1px枠と同色アイコンを常時表示する。縦方向の追加は既存要素の左ガターではなく最終行の直下へ置き、横方向の追加だけを行末へ置く。ホバーではディープインク面と白アイコンへ反転し、アイコンを160msで90度回して操作可能性を伝える。`prefers-reduced-motion`では色の反転だけを残す。
...
- **Create Name Helper:** `N / M / H / W / A / B / S / T / Q`は、作成ポップオーバー内で一文字入力欄の右に続く34px×38pxのmonospace候補にする。候補自身には枠付きコンテナ、背景、影を与えず、入力欄のホバー／フォーカス時だけ同じ面の中で現す。クリックはその一文字を最終入力として即座に追加Actionを発行する。モバイルはホバーを使わず、同じ面の下段で入力フォーカス中だけ44px以上・5列のグリッドへ展開する。
- **Create Behavior:** 作成ポップオーバーは基本型、横方向、縦方向を先に選び、名前を最後の確定トリガーにする。Rustが投影する`name_max_chars = 1`に従い、直接入力または候補選択で一文字が揃った時点で確定するためチェックボタンは置かない。成功後はポップオーバーだけを閉じ、新規記号を自動選択したり別の編集面へ遷移したりしない。
- **Existing Name Edit:** 既存記号の改名は置換操作なので、インスペクタ内の候補はdraftだけを更新し、チェックまたはEnterで明示的に適用する。作成と編集で同じチェックの意味を装わない。
...
- **Create Name:** 直接作成の名前欄はRust投影に従う一文字入力で、基本型と構造軸の後に置く。ポップオーバーを開いた直後はここへフォーカスしてキーボード入力を待ち、Escapeは未確定のまま閉じる。
...
入力形式を数式行として直接描く署名面。選択可能なのは意味のある記号であり、`role="button"`と`tabindex="0"`を持ち、クリックに加えてEnter／Spaceでも同じインスペクタを開く。非選択の記号は通常の問題文として読む。追加操作はRustが投影したbelow／right／inside／variantの方向を記法上の上下左右へそのまま写し、ディープインクの枠で常時可視にして、クリックして初めて局所エディターを開く。選択はシアンの内線、端マーク、ウォッシュを併用し、色だけに依存しない。
...
取り消し／やり直しは最大100件の履歴とfutureを持ち、履歴の有無をdisabled状態で示す。新規作成は一文字確定後に記法へ戻り、新規記号の編集はその記号を改めて選んだ時だけ開く。削除は最初のクリックで対象名と取消操作を開き、二回目でRustのRemove Actionを実行する。確定後も取り消しで戻せる。Escapeは開いているポップオーバーと選択を閉じ、`Ctrl/Cmd+Z`は履歴、`Ctrl/Cmd+Shift+Z`はfutureを進める。
...
- **Do** 新規作成は型と軸を先に決め、一文字の直接入力または名前候補クリックを最後の即時確定操作にする。
...
- **Don't** 新規作成に一文字入力後のチェックを重ねたり、作成直後に新規記号のインスペクタへ自動遷移したりしない。
- **Don't** 名前入力と名前候補を別々のポップオーバーや影付き面に分けない。
