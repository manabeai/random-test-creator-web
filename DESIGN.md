...
820px以下では58pxの固定ツールバーと58pxの表示切り替えを積み、入力形式／生成ケースのどちらか一方だけを表示する。切り替えは一回の操作で完了し、入力画面の状態を保つ。ガターは38px、入力行は74px、ツールバー操作は42px。名前候補はホバーに依存せず、一文字入力欄のフォーカス中だけ5列のグリッドとして展開し、各候補を44px以上にする。インスペクタは左右2pxまで広がり、基本型・横方向・縦方向は縦積みへ変わる。430px以下ではガター、挿入操作、seed欄をさらに詰めるが、入力形式の改行は変えない。
...
- **Create Name Helper:** `N / M / H / W / A / B / S / T / Q`は、一文字入力欄のホバー／フォーカス時だけ現れる34px×38pxのmonospace候補にする。常設の構造選択より弱い一時パレットとし、候補のクリックはその一文字を最終入力として即座に追加Actionを発行する。モバイルはホバーを使わず、入力フォーカス中だけ44px以上・5列のグリッドへ展開する。
